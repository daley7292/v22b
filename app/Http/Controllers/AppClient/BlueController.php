...
* 如果有特殊需要，则无需启用全局模式",
   ...
   public function getTempToken(Request $request)
   {
    ...
    public function appsendEmailVerify(CommSendEmailVerify $request)
    {
        ...
        $code = rand(1000, 9999);
        $subject = config('v2board.app_name', 'V2Board') . '邮箱验证码:' . $code;
    ...
    public function appnotice(Request $request)
    {
        ...
        if($lang == "en") {
          ...
          $data = [
              [
                  "created_at" => 1711378084,
                  "img_url" => "",
                  "content" => "官方公告
亲爱的用户
目前，线路为 CM A 接入香港 BGP 出港专线，ALI BGP 接入ALI BGP 香港出港专线。",
                  "tags" => ["alert"],
                  "id" => 7,
                  "updated_at" => 1725610452,
                  "title" => "官方通知",
                  "show" => 1
              ]
          ];
    ...
    public function applogin(AuthLogin $request)
    {
        ...
        if($expired_date == "长期有效") {
            ...
            $expDays = $this->getDaysDifference($expired_date);
        }
    ...
    public function appupdate(Request $request) {
        ...
        if($system == "android"){
            ...
            //test
            // if($version == "1.3.9" || $version == "1.0.0"){
            //   return response()->json([
            //       'status' => 0,
            //       'msg' => '已是最新版本'
            //   ]);
            ...
            $_nowVersion = "1.4.0"; //config('v2board.macos_version');
